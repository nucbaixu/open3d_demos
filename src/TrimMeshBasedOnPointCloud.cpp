...
int main(int argc, char* argv[])
{
    ...
    auto in_mesh_file = utility::GetProgramOptionAsString(argc, argv, "--in_mesh");
    auto out_mesh_file = utility::GetProgramOptionAsString(argc, argv, "--out_mesh");

    auto pcd_file = utility::GetProgramOptionAsString(argc, argv, "--pointcloud");
    ...
    utility::ProgressBar progress_bar(mesh->vertices_.size(),"prune vetices:");

    utility::LogInfo("vertices nums :{:d}", mesh->vertices_.size());
...
#pragma omp parallel for schedule(static) num_threads(utility::EstimateMaxThreads())
    for(int i = 0; i < (int)mesh->vertices_.size();i++)
    {
        ...
        utility::LogInfo("K and  dists :{:d},{:f}", k, dists[0]);
