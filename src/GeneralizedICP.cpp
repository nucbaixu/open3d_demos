...
int main(int argc, char* argv[]) {
	...
	for(int i = 0; i < 3; ++i)
	{
		float _voxel_size = voxel_sizes[i];
		auto source_down = source->VoxelDownSample(_voxel_size);
		source_down->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(_voxel_size * 10.0, 30));

		auto target_down = target->VoxelDownSample(_voxel_size);
		...
		auto result = pipelines::registration::RegistrationGeneralizedICP(
			*source_down, *target_down, 50, trans,
