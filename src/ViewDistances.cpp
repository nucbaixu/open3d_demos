...
int main(int argc,char *argv[])
{
    ...
    std::vector<double> distance(pcd->points_.size());
    ...
    if(utility::ProgramOptionExists(argc,argv,"--mahalanobis_distance"))
    {
        ...
        fwrite(distance.data(), sizeof(double), distance.size(), f);
        fclose(f);
    }else if(utility::ProgramOptionExists(argc, argv, "--nn_distance"))
    {
        distance = pcd->ComputeNearestNeighborDistance();
        FILE* f = utility::filesystem::FOpen(binname, "wb");
        fwrite(distance.data(), sizeof(double), distance.size(), f);
        fclose(f);
    ...
    }else
    {
        ...
        if (fread(distance.data(), sizeof(double), pcd->points_.size(), f) !=
            pcd->points_.size()) {
            utility::LogWarning("Cannot open bin file.");
            return 1;
        }
    ...
    visualization::ColorMapHot color_map;

    for(size_t i = 0; i < pcd->points_.size();i++)
    {
        pcd->colors_[i] = color_map.GetColor(distance[i]/max_distance);
    }
