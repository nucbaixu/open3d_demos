...
int main(int argc,char *argv[])
{
    ...
    *new_cloud_ptr = *cloud_ptr;
    ...
    auto new_lineset_ptr = geometry::LineSet::CreateFromPointCloudCorrespondences(*cloud_ptr, *new_cloud_ptr, correspondences);
    ...
    for (size_t i = 0; i < new_lineset_ptr->lines_.size();i++)
    {
        ...
        if((point_pair.first - point_pair.second).norm()<0.05*bounding_box.GetMaxExtent())
        {
